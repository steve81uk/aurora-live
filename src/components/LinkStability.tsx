/**
 * LinkStability — Neural Brain status indicator
 */

import { neuralForecaster } from '../ml/LSTMForecaster';

export function LinkStability() {
  const modelLoaded: boolean =
    typeof (neuralForecaster as any).isLoaded === 'boolean'
      ? (neuralForecaster as any).isLoaded
      : (neuralForecaster as any).model != null;
